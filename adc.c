...
int main(void)
{
	...
	unsigned char Msg3[11] = {"ANALOG IP:"}; unsigned char Msg4[12] = {"ADC OUTPUT:"};

	SystemInit();
	SystemCoreClockUpdate();

LPC_SC->PCONP |= (1<<15); lcd_init();		//Power for GPIO block
...
adc_temp >>= 4;
ade_temp &= 0x00000FFF;					//12 bit ADC
...
for(i=0;i<200000;i++); for(i=0;i<7;i++)
vtg[i]=dval[i] = 0x00;
ade_temp = 0;
in_vtg = 0;
}
